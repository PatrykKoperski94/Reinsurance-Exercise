...
The cumulative distribution function (CDF) of a random variable provides the probability that the variable takes on a value less than or equal to a given value. The inverse CDF, also known as the quantile function or percent-point function, calculates the value corresponding to a specific probability.
...
def rExponential(rate:float) -> float:
    # Inverse CDF of the exponential distribution
    return -math.log(1 - random.uniform(0, 1)) / rate


def rPoisson(rate:float, interval_length=1:float) -> int:
...
1. `rExponential(rate:float) -> float`:
...
2. `rPoisson(rate, interval_length=1) -> int`:
...
def rPareto(shape:float, scale:float) -> float:
...
Code explanation generated by ChatGPT:
This code snippet implements a function `rPareto(shape, scale) -> float` that generates random numbers from a Pareto distribution with given shape and scale parameters.
...
class ExcessOfLoss:
    ...
    def __post_init__(self):       
        ...
        self.claimAmountInExcess = 0.0
        self.available_aad = self.aad      
        self.recoveries = 0.0        

    def apply_treaty(self, claim_amount):
        self.claimAmountInExcess = max(claim_amount-self.deductible, 0)
        if self.claimAmountInExcess == 0:
            self.recoveries = 0
            return self

        if self.aad:
            aad = self.available_aad
            self.available_aad = max(aad - self.claimAmountInExcess, 0)
            self.claimAmountInExcess = max(self.claimAmountInExcess - aad, 0)

        recoveries = min(self.claimAmountInExcess, self.limit)
